...
export const Signup = () => {
  ...
  const handlesubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/auth/register', formData, config);
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-6 py-12">
      <div className="w-full max-w-md">
        ...
        <form className="space-y-6 bg-white p-6 rounded-lg shadow-lg" onSubmit={handlesubmit}>
          ...
          <p>already a member? <a href="http://localhost:5173/LogIn" style={{color: 'blue'}}>Log in</a></p>
        </form>
      </div>
    </div>
  );
